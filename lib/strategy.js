...
var passport = require('passport-strategy')
  ...
  , logger = require('../../../../server/utils/logging')
  //, setup = require('./setup')
...
function Strategy(options, verify) {
  ...
  this._passClaimsInfo = (options.passClaimsInfo === undefined) ? false : options.passClaimsInfo;
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
        ...
        self._shouldLoadUserProfile(iss, sub, function(err, load) {
          ...
          if (load) {
            ...
            if (self._passClaimsInfo) {
              ...
              var profile = {};
              profile.id = sub;
              profile.claims = { accesstoken: accessToken,
    ...
    this._setup(identifier, function(err, config) {
      ...
      var simple_optional_params = ['max_age', 'ui_locales', 'id_token_hint', 'login_hint', 'acr_values'];
      simple_optional_params.filter( x => { return x in config } ).map( y => { params[y] = config[y] } );

      if (req.params.mfapai) {
        // Send the MFA PAI as the login hint in Base64 format
        params.login_hint = req.params.mfapai;
      }

      if (req.params.locale) {
        // Send the MFA PAI as the login hint in Base64 format
        params.ui_locales = req.params.locale;
      }
